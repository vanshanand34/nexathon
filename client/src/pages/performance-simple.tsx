...
import { Zap, Clock, Activity, Database, ArrowRight } from "lucide-react";
import { apiRequest } from "@/lib/queryClient";
import { type AnalysisResult } from "@shared/schema";
import { useToast } from "@/hooks/simple-toast";

const PERFORMANCE_SAMPLE_CODE = `// Sample function to calculate Fibonacci numbers
...
function processData(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    for (let j = 0; j < data.length; j++) {
      if (data[i] > data[j]) {
        results.push({ first: data[i], second: data[j] });
      }
    }
  }
...
async function fetchUserData(userId) {
  // Multiple individual queries instead of a join or batch
...
export default function Performance() {
  const [code, setCode] = useState(PERFORMANCE_SAMPLE_CODE);
  ...
  const handlePerformanceAnalysis = async () => {
    ...
    try {
      const response = await apiRequest('POST', '/api/performance', {
        code,
        language,
        analysisDepth: 3
      });
      ...
      toast({
        ...
        description: `Found ${result.issuesCount} performance issues to review`,
        variant: "success"
      });
    } catch (error) {
      toast({
    ...
    <div className="container max-w-6xl px-4 py-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-6">
        <div className="border rounded-lg shadow-sm p-4">
          ...
          <div>
            <p className="text-sm text-gray-500">
              Analyze algorithm complexity and identify performance bottlenecks in your code.
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg shadow-sm p-4">
          ...
          <div>
            <p className="text-sm text-gray-500">
              Track memory consumption and CPU utilization to optimize resource efficiency.
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg shadow-sm p-4">
          ...
          <div>
            <p className="text-sm text-gray-500">
              Optimize database interactions and identify slow queries in your application.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            <div className="pt-6">
              <div className="flex flex-col space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  <span>Analyzing performance bottlenecks...</span>
                ...
                <p className="text-xs text-gray-500">
                  Our AI is analyzing your code for performance optimizations
                </p>
              </div>
            </div>
          </div>
        ...
        {analysisResult && !isAnalyzing && (
          <div className="border rounded-lg shadow-sm p-4">
            <div className="mb-4">
              <h3 className="text-lg font-medium">Performance Analysis Results</h3>
              <div className="flex items-center mt-2">
                <span className="text-sm text-gray-500 mr-2">Overall Performance Score:</span>
            ...
            <div className="mt-6">
              <h4 className="text-lg font-medium mb-4">Performance Issues</h4>
              
              {analysisResult.issues.length === 0 ? (
                <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded">
                  <div className="flex">
                    <Activity className="h-5 w-5 text-green-400 mr-2" />
                    <p className="text-sm text-green-700">No performance issues found. Your code is well-optimized!</p>
                  </div>
                </div>
              ) : (
                  ...
                  {analysisResult.issues.map((issue) => (
                    <div key={issue.id} className={`border-l-4 p-4 rounded ${
                      ...
                      {issue.suggestionCode && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-500 mb-1">Recommended optimization:</p>
